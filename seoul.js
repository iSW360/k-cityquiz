...
const LS_DAILY = 'kcityquiz_seoul_daily';
...
function handleTimeOut() {
    ...
    const btnLabel = isLast ? '결과 보기' : '다음 문제';
...
async function initMap() {
    ...
    } catch (e) { mapErrorInfo.textContent = '행정구역 데이터 로딩 실패.'; }
...
function endGame() {
    ...
    nextQuestionBtn.style.display = 'none';
    restartBtn.textContent = '처음으로';
    const dailyBanner = isDailyMode ? `<div class="flex items-center justify-center gap-2 bg-blue-600 text-white rounded-xl px-4 py-2 text-sm font-bold mb-4">📅 오늘의 서울 챌린지 완료! ${getTodayKey()}</div>` : '';
    optionsArea.innerHTML = `
      <div class="col-span-4 space-y-4">
        ...
        <div class="flex items-center justify-between bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl px-4 py-3 border border-blue-100">
          ...
          <span class="text-sm font-bold text-blue-600">${bestData ? `${bestData.score}점 · ${bestData.date}` : '-'}${isNewBest ? '<span class="ml-2 bg-yellow-400 text-white text-xs px-2 py-0.5 rounded-full">NEW!</span>' : ''}</span>
        </div>
        <div>
...
function renderWeakStats() {
    ...
    const bars = entries.map(([name, v]) => {
        ...
        return `<div><div class="flex justify-between text-xs text-gray-500 mb-0.5"><span class="font-semibold text-gray-700">${name}</span><span>${v.wrong}번 틀림 / ${total}번 출제</span></div><div class="w-full bg-gray-100 rounded-full h-2"><div class="bg-red-400 h-2 rounded-full" style="width:${wrongPct}%"></div></div></div>`;
    }).join('');
    return `<div><p class="text-sm font-bold text-gray-600 mb-2">📉 자주 틀리는 구 (누적)</p><div class="space-y-2">${bars}</div></div>`;
}
...
if (toggleGuideBtn && detailedGuide) {
    toggleGuideBtn.addEventListener('click', () => {
        ...
        if (isHidden) { detailedGuide.classList.remove('hidden'); toggleGuideBtn.textContent = '게임 상세 소개 닫기'; }
        else { detailedGuide.classList.add('hidden'); toggleGuideBtn.textContent = '상세 게임 가이드 열기'; }
    });
}
